/**
 * TraDE Middleware API
 * This is the API of the TraDE middleware.
 *
 * OpenAPI spec version: 0.0.1
 * Contact: trade4chor@gmail.com
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

import * as models from './models';

export enum ResourceTypeEnum {
    DataDependencyGraph = <any> 'DataDependencyGraph',
    DataModel = <any> 'DataModel',
    DataObject = <any> 'DataObject',
    DataElement = <any> 'DataElement',
    DataObjectInstance = <any> 'DataObjectInstance',
    DataElementInstance = <any> 'DataElementInstance',
    DataValue = <any> 'DataValue'
}
